...
const MovieDescriptionContent = ({ tagline, title, ratings, length, status, language, year, synopsis, idmbLink, webLink, genre, cast }: props) => {
 const actualrating = Math.ceil(ratings / 2 * 10) / 10
 const actualYear = useYearFromDateString(year)
  ...
  return (
    <section className='description_Content'>
     ...
     <div className='description_Content_SubDivs'>
      <DescriptionContentSubHeader header='Length' content={`${length} min`}/>
      <DescriptionContentSubHeader header='Language' content={languageName} />
      <DescriptionContentSubHeader header='Year' content={actualYear as number} />
      <DescriptionContentSubHeader header='Status' content={status} />
     </div>
     <DescriptionContentgenres genres={genre} link={'/movie/genre'} />
...
export default MovieDescriptionContent